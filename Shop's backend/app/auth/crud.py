...
async def register_user(db: AsyncSession, user_in: UserCreate):
    existing_user = await get_user_email(db, user_in.email)
    if existing_user:
...
async def auth_user(db: AsyncSession, user_in: UserCreate):
    db_user = await get_user_email(db, user_in.email)
